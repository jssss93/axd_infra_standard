...
output "key_vault_secret_ids" {
  ...
  value = {
    ...
    "cosmosdb-endpoint"      = length(azurerm_key_vault_secret.cdb_endpoint) > 0 ? azurerm_key_vault_secret.cdb_endpoint[0].id : null
    "cosmosdb-primary-key"   = length(azurerm_key_vault_secret.cdb_primary_key) > 0 ? azurerm_key_vault_secret.cdb_primary_key[0].id : null
    "cosmosdb-secondary-key" = length(azurerm_key_vault_secret.cdb_secondary_key) > 0 ? azurerm_key_vault_secret.cdb_secondary_key[0].id : null
