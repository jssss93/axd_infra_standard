...
resource "azurerm_public_ip" "this" {
  ...
  name                = var.public_ip_name != null ? var.public_ip_name : "${var.name}-${lookup(var.name_suffixes, "public_ip", "pip")}"
...
resource "azurerm_application_gateway" "this" {
  ...
  gateway_ip_configuration {
    name      = "${var.name}-${lookup(var.name_suffixes, "gateway_ip_configuration", "ip-config")}"
  ...
  dynamic "frontend_ip_configuration" {
    ...
    content {
      name                 = "${var.name}-${lookup(var.name_suffixes, "frontend_ip_public", "feip-public")}"
  ...
  dynamic "frontend_ip_configuration" {
    ...
    content {
      name                          = "${var.name}-${lookup(var.name_suffixes, "frontend_ip_private", "feip-private")}"
      subnet_id                     = var.subnet_id
  ...
  dynamic "backend_http_settings" {
    ...
    content {
      ...
      # host_name과 pick_host_name_from_backend_address는 동시에 설정 불가
  ...
  dynamic "request_routing_rule" {
    ...
    content {
      ...
      priority                    = lookup(request_routing_rule.value, "priority", 100 + (request_routing_rule.key * 10))  # 기본값: 100, 110, 120...
